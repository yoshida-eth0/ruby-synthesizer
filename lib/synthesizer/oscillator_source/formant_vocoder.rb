module Synthesizer
  module OscillatorSource
    class FormantVocoder
      ...
      def initialize(vowels: [:i, :e, :a, :o, :u])
      ...
      def next(context, delta, l_gain, r_gain)
        ...
        @pulse_context ||= @pulse.generate_context(context.soundinfo, context.phase)
        pulse = @pulse.next(@pulse_context, delta, 1.0, 1.0).streams[0]
      ...
      def generate_context(soundinfo, phase)
        FvContext.new(soundinfo, phase, Array.new(5) {|i| Vdsp::DoubleArray.new(soundinfo.window_size+2)})
      end

      FvContext = Struct.new("FvContext", :soundinfo, :phase, :tmpbufs) do
        def window_size
